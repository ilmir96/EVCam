...
public class CameraRecordingService extends Service {
        ...
        recordingHandler.post(() -> {
            try {
                ...
                if (cameraManager == null) {
                    AppLog.e(TAG, "CameraManager Не инициализация");
                    recordingController.onError(1, "CameraManager Не инициализация");
                    return;
                }
                ...
                if (cameraManager.isReleased()) {
                    AppLog.e(TAG, "КамераНе инициализация，请先打Вкл应用界面");
                    recordingController.onError(3, "КамераНе инициализация，请先打Вкл应用界面");
                    return;
                }
                ...
                if (success) {
                    ...
                    AppLog.d(TAG, "录制已Вкл始, isRecording=" + isRecording);
                } else {
                    AppLog.e(TAG, "启动Ошибка записи");
                    recordingController.onError(2, "启动Ошибка записи");
                }
            } catch (Exception e) {
                AppLog.e(TAG, "Начало записи 失败", e);
    ...
    public void stopRecording() {
        ...
        new Thread(() -> {
            try {
                ...
                AppLog.e(TAG, "СтопОшибка записи", e);
                // 确保状态重置
                mainHandler.post(() -> {
                    isRecording = false;
                    recordingController.setState(RecordingController.RecordingState.IDLE);
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            channel.setDescription("保持录制服务在фон运行");
