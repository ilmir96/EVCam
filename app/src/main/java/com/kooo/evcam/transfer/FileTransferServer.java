...
public class FileTransferServer extends NanoHTTPD {
    ...
    private Response serveFileListPage() {
        ...
        html.append("<div class=\"wechat-tip-text\">请点击Пр上角菜单<br>选择\"在浏览器打Вкл\"以下载文件</div>\n");
        html.append("</div>\n");
        html.append("</div>\n");
        ...
        html.append("<span class=\"toolbar-item\">").append(pendingFiles.size()).append("  шт.项目</span>\n");
        html.append("</div>\n");
    ...
    private String getFileBrowserScript() {
        return "<script>\n" +
            ...
            "        }\n" +
            "      } else {\n" +
            "        listEl.innerHTML = '<div style=\"padding:40px;text-align:center;\">Ошибка: ' + xhr.status + '</div>';\n" +
            "      }\n" +
            "    }\n" +
            "  };\n" +
    ...
    private Response serveFile(String fileId) {
        ...
        AppLog.i(TAG, "Вкл始传输文件: " + file.getName());
