...
public class BlindSpotFloatingWindowView extends FrameLayout {
    ...
    private void init() {
        ...
        if (closeButton != null) {
            ...
            closeButton.setOnClickListener(v -> {
                AppLog.i(TAG, "🎯 Выкл按钮被点击，发送广播ВыклНастройки");
                try {
            ...
            AppLog.w(TAG, "Выкл按钮未找到，无法设置点击监听器");
        }
        ...
        if ("left".equals(cameraPos)) {
            ...
            AppLog.i(TAG, "🎯 init()加载Л视窗配置: x=" + initialX + ", y=" + initialY +
            ...
            AppLog.i(TAG, "🎯 init()加载Пр视窗配置: x=" + initialX + ", y=" + initialY +
                    ", w=" + initialWidth + ", h=" + initialHeight);
        } else {
            // 其他情况使用转向灯补盲配置
    ...
    public void setSupervisionMode(boolean supervisionMode) {
        ...
        if (supervisionMode) {
            ...
            post(() -> {
                ...
                AppLog.d(TAG, "🎯 Настройки圆角已应用, view size=" + getWidth() + "x" + getHeight());
            });
    ...
    public void reloadSupervisionBounds() {
        ...
        AppLog.d(TAG, "当ПНастройкиПозиция大小: camera=" + cameraPos +
    ...
    private void syncPositionToPartner(int newY) {
        if (supervisionPartner != null && supervisionPartner.params != null) {
            ...
            try {
                ...
                AppLog.e(TAG, "同步调整配对窗口Позиция失败: " + e.getMessage());
            }
        }
    }
    ...
    private void saveSupervisionBounds() {
        ...
        if ("left".equals(cameraPos)) {
            ...
            AppLog.i(TAG, "🎯💾 保存Л视窗: x=" + params.x + ", y=" + params.y +
                    ", w=" + params.width + ", h=" + params.height);
        } else if ("right".equals(cameraPos)) {
            appConfig.setSupervisionRightBounds(params.x, params.y, params.width, params.height);
            AppLog.i(TAG, "🎯💾 保存Пр视窗: x=" + params.x + ", y=" + params.y +
                    ", w=" + params.width + ", h=" + params.height);
        }
    }
    ...
    private void showSupervisionAdjustDialog() {
        ...
        saveBtn.setOnClickListener(v -> {
            // 强制保存当前位置和大小
            AppLog.i(TAG, "🎯 用户点击保存，当ППозиция: x=" + params.x + ", y=" + params.y +
                    ", w=" + params.width + ", h=" + params.height);
            saveSupervisionBounds();
            hasUnsavedResize = false;
            ...
            AppLog.i(TAG, "🎯 保存З读取验证: x=" + savedX + ", y=" + savedY +
                    ", w=" + savedW + ", h=" + savedH);

            hideSupervisionAdjustPanel();
            Toast.makeText(ctx, "Позиция и размер сохранены", Toast.LENGTH_SHORT).show();
        });
        
        // 关闭按钮
        Button closeBtn = new Button(ctx);
        closeBtn.setText("Выкл");
    ...
    private void resetSupervisionBounds() {
        ...
        AppLog.i(TAG, "重置НастройкиПозиция大小为По умолчанию值");
        Toast.makeText(getContext(), "Сброшено к позиции по умолчанию", Toast.LENGTH_SHORT).show();
    }
    ...
    public void show() {
        try {
            if (this.getParent() == null) {
                ...
                // Защитный таймаут: если камера долго не присылает кадры,
                // максимум 300 мс — затем показываем напрямую
                showAnimFallback = () -> {
                    if (pendingShowAnimation) {
    ...
    private void updateCloseButtonVisibility() {
        ...
        if (closeButton != null) {
            ...
            AppLog.d(TAG, "更新Выкл按钮可见性: " + isSupervisionMode + ", 按钮=" + closeButton);
        } else {
            AppLog.w(TAG, "Выкл按钮未找到!");
        }
    }
    ...
    public void updateBounds(int x, int y, int width, int height) {
        if (params != null && windowManager != null) {
            ...
            try {
                ...
                AppLog.e(TAG, "更新悬浮窗Позиция失败: " + e.getMessage());
            }
        }
    }
    ...
    public void restartCameraPreview() {
        AppLog.i(TAG, "重新启动Камера预览: " + cameraPos);
        if (textureView != null && textureView.isAvailable()) {
            if (cachedSurface != null) {
                cachedSurface.release();
                cachedSurface = null;
