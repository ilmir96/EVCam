...
public class BlindSpotService extends Service {
    ...
    private void preCreateBlindSpotWindows() {
        if (!appConfig.isTurnSignalLinkageEnabled() && !appConfig.isLongViewModeEnabled()) {
            AppLog.d(TAG, "转向灯联动和长视模式都未Включить，跳过预创建窗口");
            return;
        }
    ...
    private void initSupervisionModeReceiver() {
        supervisionModeReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if ("com.kooo.evcam.SUPERVISION_MODE_CHANGED".equals(intent.getAction())) {
                    ...
                    AppLog.i(TAG, "🎯 Настройки广播 received, enabled=" + enabled + ", current=" + isSupervisionModeActive);
                    if (enabled) {
        ...
        AppLog.i(TAG, "🎯 Настройки广播接收器已注册 (EXPORTED)");
    }
    ...
    private void initDoorSignalObserver() {
        AppLog.i(TAG, "🚪 ========== Вкл始初始化车门联动观察者 ==========");
        AppLog.i(TAG, "🚪 补盲功能总ВклВыкл: " + appConfig.isBlindSpotGlobalEnabled());
        AppLog.i(TAG, "🚪 车门联动ВклВыкл: " + appConfig.isDoorLinkageEnabled());
        AppLog.i(TAG, "🚪 车门联动Модель: " + appConfig.getTurnSignalPresetSelection() + " (复用转向联动配置)");
        ...
        if (appConfig.isVhalGrpcTriggerMode()) {
            ...
            AppLog.w(TAG, "🚪 当П触发模式不支持车门联动: " + appConfig.getTurnSignalTriggerMode());
        }
    ...
    private void initCarSignalManagerDoorObserver() {
        ...
        doorSignalObserver = new DoorSignalObserver(this, new DoorSignalObserver.DoorSignalListener() {
            ...
            @Override
            public void onConnectionStateChanged(boolean connected) {
                AppLog.i(TAG, "🚪 车门监听连接状态: " + (connected ? "✅ Подключено" : "❌ Не подключено"));
            }
        });
    ...
    private VhalSignalObserver.DoorSignalListener createDoorSignalCallback() {
        return new VhalSignalObserver.DoorSignalListener() {
            ...
            @Override
            public void onConnectionStateChanged(boolean connected) {
                AppLog.i(TAG, "� 车辆API车门监听连接状态: " + (connected ? "✅ Подключено" : "❌ Не подключено"));
            }
        };
    }
    ...
    private void handleDoorOpen(String side) {
        AppLog.i(TAG, "🚪🚪🚪 收到车门打Вкл事件: " + side);

        if (!appConfig.isBlindSpotGlobalEnabled()) {
            AppLog.w(TAG, "🚪 补盲功能未Включить，跳过车门触发");
            return;
        }
        if (!appConfig.isDoorLinkageEnabled()) {
            AppLog.w(TAG, "🚪 车门联动未Включить，跳过车门触发");
            return;
        }

        // 如果当前有转向灯激活，车门联动让路（转向灯优先级更高）
        if (currentSignalCamera != null && !currentSignalCamera.isEmpty()) {
            AppLog.w(TAG, "🚪 转向灯正在使用(" + currentSignalCamera + ")，车门联动让 кам.");
            return;
        }

        // 如果同侧摄像头已经在显示（车门联动触发的），跳过重复显示
        if (isMainTempShown && mainFloatingWindowView != null) {
            AppLog.i(TAG, "🚪 车门联动Камера已在显示，跳过重复创建");
            // 但需要取消隐藏计时器（门重新打开了）
            if (hideRunnable != null) {
                hideHandler.removeCallbacks(hideRunnable);
                hideRunnable = null;
                AppLog.i(TAG, "🚪 Отмена隐藏计时器（门重新打Вкл）");
            }
            return;
        }

        AppLog.i(TAG, "🚪 ✅ 车门打Вкл: " + side + "，准备显示Камера");

        // 如果超视模式激活，保存状态并关闭它
        if (isSupervisionModeActive) {
            AppLog.i(TAG, "🚪 Настройки激活中，保存状态并Выкл，显示车门补盲");
    ...
    private void handleDoorClose(String side) {
        AppLog.i(TAG, "🚪🚪🚪 收到车门Выкл事件: " + side);

        if (!appConfig.isDoorLinkageEnabled()) {
            AppLog.w(TAG, "🚪 车门联动未Включить，跳过Выкл逻辑");
            return;
        }

        // 只有在没有转向灯激活时才关闭车门摄像头
        if (currentSignalCamera != null && !currentSignalCamera.isEmpty()) {
            AppLog.w(TAG, "🚪 转向灯正在使用(" + currentSignalCamera + ")，不Выкл车门Камера");
            return;
        }

        // 检查是否有车门联动触发的窗口在显示
        if (!isMainTempShown && dedicatedBlindSpotWindow == null) {
            AppLog.i(TAG, "🚪 没有车门联动窗口在显示，跳过Выкл逻辑");
            return;
        }

        AppLog.i(TAG, "🚪 ✅ 车门Выкл: " + side + "，准备延迟ВыклКамера");
        startDoorHideTimer();
    }
    ...
    private void showBlindSpotCamera(String cameraPos) {
        ...
        if (isSupervisionModeActive) {
            AppLog.d(TAG, "Настройки激活中，忽略转向灯信号: " + cameraPos);
            return;
        }

        AppLog.i(TAG, "🚦 转向灯触发Камера: " + cameraPos);
        
        // 如果车门联动窗口在显示，先关闭（转向灯优先级更高）
        if (isMainTempShown) {
            AppLog.i(TAG, "🚦 检测到车门联动窗口，转向灯接管（优先级更Высокое）");
        ...
        AppLog.i(TAG, "🚦 转向灯激活，Настройки currentSignalCamera = " + cameraPos);
    ...
    private void handleTurnSignal(String cameraPos) {
        ...
        AppLog.d(TAG, "转向灯触发Камера: " + cameraPos + " [T+0ms]");
        ...
        // --- 2. 异步启动前台服务和初始化相机（与 UI 布局并行） ---
        CameraForegroundService.start(this, "Слепые зоны активны", "Отображение мониторинга слепых зон");
        AppLog.d(TAG, "активно服务启动请求 [T+" + (System.currentTimeMillis() - t0) + "ms]");
        ...
        // --- 3. 打开相机（等待前台服务就绪） ---
        {
            ...
            if (cm != null) {
                ...
                if (cam != null && !cam.isCameraOpened()) {
                    CameraForegroundService.whenReady(this, cam::openCameraDeferred);
                    AppLog.d(TAG, "📷 请求打Вкл相机(whenReady) [T+" + (System.currentTimeMillis() - t0) + "ms]");
                } else if (cam != null) {
                    AppLog.d(TAG, "📷 相机открыть [T+" + (System.currentTimeMillis() - t0) + "ms]");
                }
            }
        }
    ...
    private void handleLongViewModeTurnSignal() {
        AppLog.i(TAG, "🎯 长视模式：打转向灯，显示ЛПр双画面");
        ...
        // 启动前台服务
        CameraForegroundService.start(this, "长视模式运行中", "正在显示ЛПр补盲画面");
    ...
    private void startSecondaryCameraPreviewDirectly(String cameraPos) {
        ...
        if (newCamera == secondaryCamera && surfaceReady && newCamera.isSecondaryDisplaySurfaceBound(secondaryCachedSurface)) {
            ...
            AppLog.d(TAG, "副屏Камера未变化且 Surface 已绑定，跳过 Session 重建: " + cameraPos);
            return;
        }
        ...
        if (secondaryCamera != null && secondaryTextureView != null && secondaryTextureView.isAvailable()) {
                ...
                if (previewSize == null || !secondaryCamera.isCameraOpened()) {
                    ...
                    if (!cam.isCameraOpened()) {
                        ...
                        if (!mainWindowWillOpenCamera) {
                            AppLog.d(TAG, "副屏主动打Вкл相机（无主屏窗口触发）: " + cameraPos);
                            CameraForegroundService.whenReady(BlindSpotService.this, cam::openCamera);
                        } else {
                            AppLog.d(TAG, "副屏等待主屏窗口打Вкл相机（避免过早创建session）: " + cameraPos);
                        }
                    }
                    return;
                }
            ...
            if (isSwitchingCamera) {
                ...
                AppLog.d(TAG, "副屏延迟绑定 Surface（等待旧 session Выкл）: " + cameraPos);
    ...
    private void preOpenCamera(String cameraPos) {
        ...
        if (cam != null && !cam.isCameraOpened()) {
            AppLog.d(TAG, "预触发相机打Вкл（与UI并行）: " + cameraPos);
    ...
    private void startHideTimer() {
        ...
        AppLog.i(TAG, "🚦 转向灯熄灭，启动隐藏计时器: " + timeout + "秒ЗВыклКамера");
    ...
    private void showDoorCamera(String side) {
        ...
        if (isSupervisionModeActive) {
            AppLog.d(TAG, "Настройки激活中，忽略车门信号: " + side);
            return;
        }

        AppLog.i(TAG, "🚪 ========== showDoorCamera Вкл始执行 ==========");
        ...
        // 副屏摄像头预览（复用转向联动的配置）
        if (appConfig.isSecondaryDisplayEnabled()) {
            AppLog.d(TAG, "🚪 启动副屏Камера预览: " + side);
    ...
    private void startDoorHideTimer() {
        ...
        int timeout = appConfig.getTurnSignalTimeout();
        AppLog.i(TAG, "🚪 车门Выкл，启动隐藏计时器: " + timeout + "秒ЗВыклКамера (复用转向联动配置)");
        ...
        hideRunnable = () -> {
            ...
            if (wasSupervisionModeActiveBeforeDoor) {
                AppLog.i(TAG, "🚪 恢复Настройки（车门打ВклП处于激活状态）");
                wasSupervisionModeActiveBeforeDoor = false;
                startSupervisionMode();
            }
    ...
    private void closeCamerasIfIdle() {
        ...
        // 超视模式激活时，保持相机连接
        if (isSupervisionModeActive) {
            AppLog.d(TAG, "Настройки激活中，保持相机连接");
            return;
        }
        MultiCameraManager cameraManager = com.kooo.evcam.camera.CameraManagerHolder.getInstance().getCameraManager();
        if (cameraManager != null) {
    ...
    private void startSupervisionMode() {
        if (isSupervisionModeActive) {
            AppLog.d(TAG, "Настройки已经在运行中");
            return;
        }

        AppLog.i(TAG, "🎯 启动Настройки");
        isSupervisionModeActive = true;

        // 用户手动启动超视模式，重置车门恢复标志（避免自动恢复冲突）
        if (wasSupervisionModeActiveBeforeDoor) {
            AppLog.d(TAG, "🎯 用户手动启动Настройки，重置车门恢复标志");
            wasSupervisionModeActiveBeforeDoor = false;
        }
        ...
        AppLog.i(TAG, "🎯 Настройки已启动 - ЛПр双视窗");
    }
    ...
    private void updateSupervisionWindowBounds() {
        ...
        if (leftWidth != rightWidth || leftHeight != rightHeight) {
            AppLog.w(TAG, "🎯 ЛПр窗口大小不一致，同步为Л窗口大小: w=" + leftWidth + ", h=" + leftHeight);
    ...
    private void updateWindows() {
        // 如果超视模式激活，保持运行
        if (isSupervisionModeActive) {
            CameraForegroundService.start(this, "Настройки运行中", "正在显示ЛПр补盲画面");
            return;
        }
    ...
    private void showSecondaryDisplay() {
        ...
        secondaryTextureView.setSurfaceTextureListener(new TextureView.SurfaceTextureListener() {
            ...
            public void onSurfaceTextureAvailable(android.graphics.SurfaceTexture surface, int w, int h) {
                ...
                if (cameraPos == null) {
                    AppLog.d(TAG, "副屏 Surface 就绪，但未Включить视频输出");
                    return;
                }
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        try {
                            ...
                            AppLog.e(TAG, "更新模拟悬浮窗Позиция失败: " + e.getMessage());
                        }
                        return true;
    ...
    private void checkAvmForeground() {
        ...
        if (isAvmForeground || selfFg) {
            ...
            if (!isAvmAvoidanceActive) {
                isAvmAvoidanceActive = true;
                AppLog.i(TAG, "全景影像避让：隐藏主屏补盲窗口（AVM=" + isAvmForeground + ", 自身активно=" + selfFg + "）");
                if (mainFloatingWindowView != null) {
                    mainFloatingWindowView.dismiss();
                    mainFloatingWindowView = null;
                }
            ...
            avmDeactivateCount++;
            AppLog.d(TAG, "全景影像避让：未检测到активно (" + avmDeactivateCount + "/" + AVM_DEACTIVATE_THRESHOLD + ")");
            if (avmDeactivateCount >= AVM_DEACTIVATE_THRESHOLD) {
                isAvmAvoidanceActive = false;
                avmDeactivateCount = 0;
                AppLog.i(TAG, "全景影像避让：" + targetActivity + " 已离Вклактивно，恢复主屏补盲窗口");
                updateMainFloatingWindow();
            }
        }
    }
    ...
    private boolean isActivityInForeground(String activityClassName) {
        try {
            ...
            AppLog.e(TAG, "检测активноActivity失败: " + e.getMessage());
        }
        return false;
    }
    ...
    private boolean isPackageInForeground(String packageName) {
        try {
            ...
            AppLog.e(TAG, "检测активно包名失败: " + e.getMessage());
        }
        return false;
    }
